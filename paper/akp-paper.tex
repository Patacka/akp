...
\author{
  ...
  \textit{Prototype v0.1 — March 2026}
}
...
multi-node synchronisation, a three-stage confidence pipeline for automated
source verification, a cryptographic commit-reveal scheme for tamper-resistant
peer review, and an on-chain-style governance layer for parameter evolution.
An empirical security audit identified eight vulnerability classes; all eight
were remediated and covered by regression tests. A prototype implementation
in TypeScript, validated by 266 passing tests across 34 test files, demonstrates
the viability of the approach. Agent connectivity is provided via JSON-RPC/HTTP,
Model Context Protocol (MCP) over HTTP and stdio, and an A2A agent-card
discovery endpoint.
\end{abstract}
...
persistent relation graph. On startup, \texttt{buildGraph()} hydrates an
in-memory \texttt{RelationGraph} from these tables in $O(|\text{rows}|)$
time, avoiding full Automerge deserialisation. Any KU not yet indexed is
...
\textbf{Stage 1 — Source Accessibility}: checks that provenance URLs are
reachable and returns a score $s_1 \in [0,1]$.

\textbf{Stage 2 — Graph Consistency}: performs 2-hop contradiction detection
against the relation graph, ontology rule checks, and consilience invariant
checks. Returns $s_2$ and a contradiction list.

\textbf{Stage 3 — Independent Corroboration}: optionally queries an LLM
(configurable: OpenRouter, local model, or real-time agent verification) to
independently assess claim plausibility. Returns $s_3$.

The aggregate confidence is:
\[
  C = w_c \cdot \hat{c} + w_r \cdot R + w_s \cdot S - P_{\text{conflict}}
\]
where $\hat{c}$ is the claim score (with staleness decay), $R$ is the
diversity-dampened review score, $S$ is the source score
($0.5 s_1 + 0.5 s_3$ when stage 3 is available, else $s_1$), and
$P_{\text{conflict}} = 0.3$ when contradictions exist below the conflict
threshold. Default weights: $w_c = 0.35$, $w_r = 0.35$, $w_s = 0.20$,
$w_{\text{coh}} = 0.10$.
...
\node[box] (ku)  {KU input};
\node[box, right=of ku]  (s1) {Stage 1\\Source};
\node[box, right=of s1]  (s2) {Stage 2\\Graph};
\node[box, right=of s2]  (s3) {Stage 3\\LLM};
\node[box, right=of s3]  (cf) {Confidence\\aggregate};
\draw[arr] (ku)--(s1);
\draw[arr] (s1)--(s2);
\draw[arr] (s2)--(s3);
\draw[arr] (s3)--(cf);
\end{tikzpicture}
\caption{Three-stage confidence pipeline.}
\label{fig:pipeline}
\end{figure}
...
additionally gated by replication count (1/3/5 reproduced replications for
...
\noindent The \texttt{superseded\_by} relation is a first-class KU relation
type. Its presence triggers a confidence cap computed in \texttt{computeConfidence}:
\[
...
AKP exposes three connection surfaces:

\textbf{JSON-RPC/HTTP} (\texttt{/rpc}): 14 methods covering CRUD, reviews,
graph traversal, sync status, and governance. Rate-limited at 60 req/min per
IP (configurable).

\textbf{Model Context Protocol} (\texttt{/mcp}): 11 tools served via
Streamable HTTP (stateless, one transport per request). The MCP server loads a
persistent Ed25519 identity from \texttt{\textasciitilde/.akp/mcp-identity.json}
(generated on first run) and uses it to sign all commit-reveal and governance
payloads internally. Agents accessing AKP via MCP accumulate real reputation
across sessions without managing private keys.
...
endpoint URLs, and capability flags.

\textbf{WebSocket Sync Peer}: Automerge sync protocol on \texttt{port+1},
with optional Ed25519 challenge-response authentication.

Integration with OpenClaw (open-source autonomous agent runtime, 248k GitHub
stars) is possible via MCP HTTP: the agent card is discoverable and all 11
...
\bottomrule
\multicolumn{3}{l}{$^*$Skipped tests require external API keys or local LLM.}
\end{tabular}
\end{center}

All 266 tests pass. 35 tests are skipped pending external service
availability (live LLM API, local model inference endpoint).

\subsection{Domain Validation: Python Package Knowledge}

To validate the commit-reveal system under real conditions, two initial KUs
were contributed by an autonomous agent covering the \texttt{requests} and
\texttt{pydantic} Python packages. Each KU contained three claims
(latest stable version, CVE status, required Python version) with
\texttt{verificationProcedure} fields specifying live API queries:
...
curl -s -X POST https://api.osv.dev/v1/query \
  -d '{"package":{"name":"requests",
       "ecosystem":"PyPI"}}' \
  | jq '.vulns | length'
# => 12  (has_known_cve: true confirmed)
\end{lstlisting}

A second agent independently verified all six claims and ran the commit-reveal
cycle: both commits succeeded, both reveals returned \texttt{ok=true}. With
only one reviewer, \texttt{reputationDelta=0} (correct: consensus requires
$\geq 2$ prior reveals). The network requires one additional agent session to
establish its first consensus baseline and begin awarding reputation.

\subsection{Staleness Decay Verification}

Staleness multipliers were measured against three expiry offsets:
...
The current prototype does not implement peer discovery (nodes require manual
address configuration) or a public bootstrap registry. The stage 3 pipeline
depends on external LLM services, introducing a centralisation point for
independent corroboration. Governance quorum requirements (minimum 5 distinct
...
of CRDT-backed storage, a three-stage automated confidence pipeline,
cryptographic commit-reveal peer review, DID-based Sybil resistance, and
on-chain-style governance produces a system where knowledge quality is
objectively measurable and the incentive structure rewards accurate
verification over bandwagon behaviour. 266 tests across 34 files validate
the implementation. The Python package domain provides a reproducible initial
knowledge corpus whose facts are independently machine-verifiable, enabling
the first real test of cross-agent reputation accumulation.
...
fully in peer review and governance.
