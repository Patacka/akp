...
A three-node cluster is reproducible via Docker Compose:
\begin{lstlisting}
...
changing application code. E5's 44.4\% staleness detection accuracy is
attributed to prompt design, not provider dependence: the fix (injecting
claim age and domain recency heuristics into the Stage 3 prompt) is
independent of which backend is used.

Latency is a real trade-off. A local 9B model adds approximately 30--60\,s
per KU review at the inference rates observed in E1--E9. AKP mitigates this
by running stage 3 asynchronously after the commit-reveal cycle rather than
blocking KU creation.

\subsection{Scalability Bounds and Known Gaps}

\textbf{Storage}: SQLite is a single-writer database. Each AKP node maintains
its own SQLite file; there is no shared-write bottleneck. Write throughput is
bounded by SQLite's WAL mode performance ($\sim$10\,k writes/s on commodity
SSD). At 1\,KiB per Automerge snapshot and one write per KU update, a node
handling 1\,000 KU updates/min consumes approximately 60\,MB/min of WAL
before checkpoint. This is manageable for the anticipated prototype workload
but may require alternative storage backends (e.g., PostgreSQL via a
compatibility layer) at production scale.

\textbf{Graph traversal}: The SQLite-backed \texttt{RelationGraph} uses
recursive CTEs for BFS. At $10^5$ nodes, BFS to depth 3 touches at most
$10^{5}$ rows (full traversal). Benchmark measurements show $<$50\,ms for
3-hop BFS on a graph of this size on a modern laptop SSD.

\textbf{Network}: Automerge is AP under the CAP theorem—nodes operate
independently during network partitions and converge on reconnect. Partition
handling is therefore provided by the CRDT layer at no additional cost. What
has \emph{not} been evaluated: cross-continent gossip latency, throughput
under concurrent peer connections ($>$10 nodes), or behaviour when a node
rejoins after a long partition with a large delta backlog. These are open
empirical questions.
...
manual address configuration). Governance quorum requirements (minimum 5
DIDs) cannot be met by a single-operator deployment without development-mode
overrides. Write throughput, geographic latency, and large-partition recovery
have not been measured; these are priorities for the v0.2 evaluation.
